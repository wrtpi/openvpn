#!/bin/bash
#
# https://github.com/WRTPI/openvpn
#
# Copyright (c) 2021 WRTPI. Released under the MIT License.
...
支持的发行版包括 Ubuntu、Debian、CentOS 和 Fedora。"
	exit
fi
...
if ! grep -q sbin <<< "$PATH"; then
	echo '$PATH 不包括 sbin。 尝试使用“su -”代替"su".'
	exit
fi
...
verb 3" > /etc/openvpn/server/client-common.txt
	...
	case "$option" in
		1)
			...
			new_client
			echo
			echo "$client 添加。 可用配置在:" ~/"$client.ovpn"
			exit
		;;
		2)
